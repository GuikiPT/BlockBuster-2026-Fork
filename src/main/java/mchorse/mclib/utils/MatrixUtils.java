...
/**
 * Port of McLib 2.4.3's MatrixUtils (roadmap P13) — the pure-math half.
 *
 * Legacy source: .tools/legacy-src/mclib/src/main/java/mchorse/mclib/utils/MatrixUtils.java
 *
 * The GL-state-reading members of the legacy class have no core-profile
 * equivalent on 1.20.4 and are NOT ported here; they land in S6 rendering
 * where the current matrix is the {@code MatrixStack} argument:
 *
 * - {@code readModelView(...)} / {@code readModelViewDouble()} /
 *   {@code loadModelView(...)} (read/write {@code GL_MODELVIEW_MATRIX});
 * - the {@code camera} matrix + {@code readCamera()} / {@code ASMAfterCamera()}
 *   / {@code getCameraMatrix()} (fed by an ASM hook after camera setup);
 * - the no-arg {@code getTransformation()} (depends on the captured camera
 *   matrix and {@code Minecraft.getRenderViewEntity()}).
 *
 * {@link #captureMatrix(Matrix4f)} keeps the legacy single-slot capture
 * semantics but is fed a matrix by the caller instead of reading GL state;
 * S6 render code supplies the MatrixStack-derived model-view matrix.
 */
public class MatrixUtils
{
    /**
     * Float array for transferring data between buffers and matrices
     */
    ...
    /**
     * Single-slot capture (legacy {@code captureMatrix()} read the GL model
     * view; here the caller passes the current model-view matrix in).
     */
    public static boolean captureMatrix(Matrix4f modelView)
    {
        if (matrix == null)
        {
            matrix = new Matrix4f(modelView);
