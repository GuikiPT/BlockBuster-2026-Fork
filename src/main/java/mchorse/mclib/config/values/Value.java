...
/**
 * Port of McLib 2.4.3's {@code config/values/Value.java} (roadmap P19).
 *
 * <p><b>Client GUI split (applies to the whole {@code config.values} package):</b>
 * legacy value classes implemented {@code IConfigGuiProvider.getFields(Minecraft,
 * GuiConfigPanel)} and built their own GUI widgets. In the port, value classes
 * carry <i>no</i> GUI interface at all — GUI widget selection moves to a
 * client-side per-type factory registry that lands with S3
 * ({@code mchorse.mclib.client.gui.config}). Everything else ports verbatim.</p>
 *
 * Legacy source: .tools/legacy-src/mclib/build/sources/main/java/mchorse/mclib/config/values/Value.java
 */
public class Value implements IByteBufSerializable, ICopy<Value>
{
    ...
    /* Lang key helpers (legacy @SideOnly(CLIENT); harmless strings, kept in main) */

    /**
     * Legacy dereferenced {@code config} unconditionally, which is safe in game
     * (a value only reaches a GUI after its module was registered) but turns
     * every headless widget construction into an NPE when the module was never
     * built. An unregistered value simply has no lang key.
     */
    public String getLabelKey()
    {
        return this.config == null ? "" : this.config.getValueLabelKey(this);
    }

    public String getCommentKey()
    {
        return this.config == null ? "" : this.config.getValueCommentKey(this);
    }
}
