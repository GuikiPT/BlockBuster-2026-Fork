...
/**
 * Port of McLib 2.4.3's {@code KeyParser} (roadmap P8). Forge's
 * {@code ByteBufUtils.read/writeUTF8String} (varint length + UTF-8 bytes) maps
 * to yarn {@code PacketByteBuf.readString/writeString} — identical wire shape.
 */
public class KeyParser
{
    ...
    {
        return new PacketByteBuf(buffer).readString();
    }

    private static void writeUTF8String(ByteBuf buffer, String string)
    {
        new PacketByteBuf(buffer).writeString(string);
    }
    ...
    public static IKey keyFromBytes(ByteBuf buffer)
    {
        ...
        if (type == 0)
        {
            return IKey.str(readUTF8String(buffer));
        }
        else if (type == 1)
        {
            String key = readUTF8String(buffer);
            ...
            for (int i = 0, c = buffer.readInt(); i < c; i++)
            {
                ...
                if (argType == 0)
                {
                    args.add(readUTF8String(buffer));
                }
                else if (argType == 1)
                {
    ...
    public static void keyToBytes(ByteBuf buffer, IKey key)
    {
        ...
        {
            buffer.writeByte(0);
            writeUTF8String(buffer, ((StringKey) key).string);
        }
        else if (key instanceof LangKey)
        {
            ...
            buffer.writeByte(1);
            writeUTF8String(buffer, lang.key);
            ...
            for (Object arg : lang.args)
            {
                ...
                {
                    buffer.writeByte(0);
                    writeUTF8String(buffer, (String) arg);
                }
                ...
                {
                    buffer.writeByte(0);
                    writeUTF8String(buffer, arg.toString());
                }
            }
        }
