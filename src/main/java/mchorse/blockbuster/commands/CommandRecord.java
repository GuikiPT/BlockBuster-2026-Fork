...
/**
 * Command /record — port of legacy {@code commands.CommandRecord} (P121/P122).
 *
 * This command provides an interface which allows the manipulation of player
 * recordings on the server.
 *
 * <p>Parity notes: legacy registered {@code SubCommandRecordDelete} twice
 * (harmless LinkedHashMap overwrite keeping the first position) — the port
 * adds it once at the first legacy position. {@code camera} was registered
 * only when Aperture was installed; Aperture is bundled now, so it is always
 * registered.</p>
 */
public class CommandRecord extends SubCommandBase
{
    ...
    {
        /* Register sub-commands (legacy order) */
