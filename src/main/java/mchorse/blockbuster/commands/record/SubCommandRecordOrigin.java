...
public class SubCommandRecordOrigin extends SubCommandRecordBase
{
    ...
    public void executeCommand(MinecraftServer server, ServerCommandSource sender, String[] args) throws CommandException
    {
        ...
        Vec3d position = sender.getPosition();
        ...
        double rotation = args.length >= 2 ? parseDouble(args[1]) : 0;
        ...
        for (Frame frame : record.frames)
        {
            if (i == 0)
            {
                ...
                if (args.length >= 5)
                {
                    x = parseDouble(firstX, args[2], false);
                    y = parseDouble(firstY, args[3], false);
                    z = parseDouble(firstZ, args[4], false);
                }
            }
