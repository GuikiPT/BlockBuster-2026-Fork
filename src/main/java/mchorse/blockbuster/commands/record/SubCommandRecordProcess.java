 ...
 *
 * This command is responsible for processing given property according to
 * given math expression.
 *
 * Quirk (kept): {@code factor} divides by {@code (end - start)} — equal
 * bounds yield NaN, exactly like 1.12.2.
 */
public class SubCommandRecordProcess extends SubCommandRecordBase
{
    ...
    public void executeCommand(MinecraftServer server, ServerCommandSource sender, String[] args) throws CommandException
    {
        ...
        start = parseInt(args[2], start, end);
        end = parseInt(args[3], start, end);
