...
/**
 * Packet of frame ranges, split up in chunks to avoid the max packet size
 * error (roadmap P115). Prepends {@code from}, {@code to}, {@code index},
 * {@code callbackID} ints before the base {@link PacketFrames} fields, and
 * implements the McLib {@link IAnswerRequest} contract so the server answers
 * via {@link PacketAnswer} (P26 answer/callback infra).
 */
public class PacketFramesOverwrite extends PacketFrames implements IAnswerRequest<SimpleEntry<IKey, Boolean>>
{
    ...
    /**
     * @param value expects {IKey, boolean}
     * @return the packet answer
     * @throws NoSuchElementException if {@link #getCallbackID()} is not present
     */
    @Override
