 ...
 *
 * <p>Direct port of Blockbuster 2.7.2's {@code api/formats/vox/VoxReader.java}
 * (roadmap P66). {@code javax.vecmath.Matrix3f} is replaced by
 * {@code org.joml.Matrix3f}; see {@link #readRotation(int)} for the
 * row/column-index remapping this requires.</p>
 *
 * <p><b>Totality boundary:</b> bad magic and a non-{@code MAIN} first chunk
 * <b>throw</b> here on purpose; total-loading (skip-one-model-not-the-reload)
 * is provided one level up by {@code ModelHandler}'s per-model try/catch
 * (roadmap P69). Unknown chunks are skipped by size and the read loop
 * terminates on EOF-exception, so a truncated file simply stops.</p>
 *
...
public class VoxReader extends BinaryReader
{
    ...
    /**
     * Decode a MagicaVoxel packed rotation byte into a signed permutation
     * matrix.
     *
     * <p>The legacy code used {@code javax.vecmath.Matrix3f} whose
     * {@code setElement(row, column, value)} and {@code mRC} fields are
     * <b>row-major</b>. JOML's {@code Matrix3f} is <b>column-major</b>: its
     * field {@code mCR} is column {@code C}, row {@code R}. To keep the same
     * mathematical matrix we start from a zeroed matrix (JOML's default ctor is
     * identity, vecmath's was zero) and set element (row, col) via
     * {@link #setElement(Matrix3f, int, int, float)}, which maps to the JOML
     * field {@code m<col><row>}. Verified with hand-decoded examples (e.g. the
     * packed byte {@code 0b0000100} decodes to identity).</p>
     */
    public Matrix3f readRotation(int rotation)
    {
        ...
        setElement(matrix, 0, firstIndex, negativeFirst ? -1 : 1);
        setElement(matrix, 1, secondIndex, negativeSecond ? -1 : 1);
        setElement(matrix, 2, thirdIndex, negativeThird ? -1 : 1);

        return matrix;
    }

    /**
     * Set the element at {@code (row, column)} of a JOML (column-major)
     * {@link Matrix3f}, mirroring {@code javax.vecmath.Matrix3f.setElement(row,
     * column, value)}. JOML stores the value at field {@code m<column><row>}.
     */
    private static void setElement(Matrix3f matrix, int row, int column, float value)
    {
        switch (column * 3 + row)
        {
            case 0: matrix.m00 = value; break;
            case 1: matrix.m01 = value; break;
            case 2: matrix.m02 = value; break;
            case 3: matrix.m10 = value; break;
            case 4: matrix.m11 = value; break;
            case 5: matrix.m12 = value; break;
            case 6: matrix.m20 = value; break;
            case 7: matrix.m21 = value; break;
            case 8: matrix.m22 = value; break;
        }
    }
}
