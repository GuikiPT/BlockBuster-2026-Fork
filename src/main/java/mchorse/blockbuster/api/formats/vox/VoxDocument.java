...
/**
 * Direct port of Blockbuster 2.7.2's {@code api/formats/vox/VoxDocument.java}
 * (roadmap P66).
 *
 * <p><b>Porting notes (javax.vecmath -&gt; JOML):</b> the legacy scene-graph
 * walk pulled the rotational 3x3 and translation out of the per-frame
 * {@code Matrix4f} by copying its row-major {@code m03/m13/m23} translation
 * fields. JOML is column-major, so translation lives in {@code m30/m31/m32};
 * this port uses {@link Matrix4f#getTranslation(Vector3f)} and
 * {@code new Matrix3f(Matrix4fc)} to extract the same mathematical values.
 * Rotation composition ({@code parentMat.mul(mat)}) and translation composition
 * ({@code parentVec.add(vec)}) match vecmath 1:1. The {@code index} numbering
 * is preserved exactly — limb names are stable identifiers referenced by users'
 * pose NBT.</p>
 */
public class VoxDocument
{
    ...
    private void generateNodes(VoxTransform transform, List<LimbNode> nodes, Stack<Matrix3f> matStack, Stack<Vector3f> vecStack)
    {
        ...
        /* Extract the rotational 3x3 and the translation of this frame. In JOML
         * (column-major) these come from the upper-left 3x3 and m30/m31/m32
         * respectively — the mathematical equivalent of the legacy vecmath
         * m00..m22 / m03/m13/m23 field reads. */
        Matrix3f mat = new Matrix3f(trans);
        Vector3f vec = trans.getTranslation(new Vector3f());
        ...
        {
            Matrix3f shapeMat = matStack.pop();
            Vector3f shapeVec = vecStack.pop();

            if (!hidden)
            {
                Vox chunk = this.chunks.get(((VoxShape) child).modelAttrs.get(0).id);

                nodes.add(new LimbNode(chunk, shapeMat, shapeVec, name));
            }
        }
    }
