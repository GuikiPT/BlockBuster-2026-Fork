 ...
 * Model pose adapter
 *
 * Serializer-only: pose deserialization runs through plain Gson field names, so
 * the JSON keys must exactly match {@link ModelTransform}'s field names
 * ({@code translate}/{@code scale}/{@code rotate}). Note {@link #isDefault} uses
 * exact float equality (no epsilon), unlike {@link ModelTransform#equalFloatArray}.
 */
