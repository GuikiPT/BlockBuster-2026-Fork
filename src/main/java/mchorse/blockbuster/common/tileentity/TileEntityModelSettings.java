...
/**
 * Model-block settings serializer (roadmap P95).
 *
 * <p>Verbatim port of 1.12.2
 * {@code common/tileentity/TileEntityModelSettings.java}. Preserved
 * load-bearing behavior:</p>
 * <ul>
 * <li>the exact {@link ValueSerializer} registration order and NBT key names —
 * the disk contract for {@code model} block entities and their
 * {@code BlockEntityTag} item NBT;</li>
 * <li>the {@code Scale} NBT key is the <b>uniform-scale boolean</b> (never a
 * factor); {@code ScaleX/Y/Z} carry the factors (default 1);</li>
 * <li>default-valued settings write <b>no NBT keys at all</b> (the serializer
 * skips values whose {@code hasChanged()} is false) — this is what lets
 * {@link BlockModel#setTENBTtoStack} keep default model-block items stackable.</li>
 * </ul>
 *
 * <p>Port note: the legacy class also implemented {@code ITransformationObject}
 * and carried {@code addTranslation(...)} for the GUI transform drag handles.
 * That interface + {@code TransformOrientation} live in the split <b>client</b>
 * source set (GUI-only); the transform-drag surface is added with the model
 * dashboard — SEAM(P100). The data core (NBT/byte-buf/copy/getters/setters)
 * ported here is what the block entity + sync packet need.</p>
 */
public class TileEntityModelSettings implements IByteBufSerializable, INBTSerializable, ICopy<TileEntityModelSettings>
{
    ...
    @Override
    public void fromNBT(NbtCompound nbtTagCompound)
    {
        this.serializer.fromNBT(nbtTagCompound);
    }

    @Override
    public NbtCompound toNBT(NbtCompound nbtTagCompound)
    {
