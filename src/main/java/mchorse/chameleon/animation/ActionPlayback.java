...
/**
 * One playing animation: its tick cursor, speed, looping and fade state.
 *
 * <p>Fading is counted in whole ticks down from {@code config.fade}, and
 * {@link #getFadeFactor} turns that counter plus the partial tick into the
 * 0..1 blend weight the {@link Animator} passes to
 * {@link ChameleonAnimator#animate}. The factor is <b>inverted</b> between the
 * two directions: fading out runs 1 → 0, fading in runs 0 → 1.</p>
 *
 * <p>Port note: legacy was {@code @SideOnly(CLIENT)} only because
 * {@link ChameleonAnimator} was; nothing here is client-only, and
 * {@code ChameleonMorph} (common source set) holds these objects directly.</p>
 *
 * Legacy source: chameleon/.../animation/ActionPlayback.java
 */
public class ActionPlayback
{
    ...
    public void apply(LivingEntity target, Model armature, float partialTick, float blend, boolean skipInitial)
    {
