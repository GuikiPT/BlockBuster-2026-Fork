...
/**
 * Parses one entry of a {@code .animation.json}'s {@code animations} object into
 * an {@link Animation}.
 *
 * <p>Handles all four shapes Blockbench/GeckoLib emit for a channel: a bare
 * {@code [x, y, z]} vector (a single keyframe at t=0), a {@code {"vector": …}}
 * object, and a time-keyed map whose values are either vectors or
 * {@code {"pre": …, "post": …}} pairs. A time key that is not a number is
 * skipped rather than failing the animation.</p>
 *
 * <p>Each axis value is either a JSON number — wrapped in a constant — or a
 * Molang string parsed against the shared parser, which is why
 * {@link mchorse.chameleon.Chameleon#PARSER} must be the same instance that
 * later evaluates it.</p>
 *
 * <p>Interpolation comes from {@code "lerp_mode": "catmullrom"} (hermite) or
 * GeckoLib's {@code "easing"} name; {@code pre}/{@code post} pairs are what make
 * step/discontinuous keyframes work (Chameleon 1.2.2).</p>
 *
 * Legacy source: chameleon/.../lib/parsing/AnimationParser.java
 */
