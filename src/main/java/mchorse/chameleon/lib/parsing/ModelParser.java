...
/**
 * Parses a Bedrock {@code .geo.json} into a {@link Model}.
 *
 * <p><b>Only the first geometry is read.</b> {@code minecraft:geometry[0]} —
 * a file declaring several geometries contributes exactly one model, as legacy.</p>
 *
 * <p>Three coordinate flips are applied here and nowhere else, and every
 * downstream transform assumes they have already happened:</p>
 * <ul>
 *   <li>bone pivot X and cube origin/pivot X are negated (Bedrock is +X-left,
 *       Minecraft is +X-right),</li>
 *   <li>bone and cube rotation X and Y are negated,</li>
 *   <li>a cube with no explicit {@code pivot} pivots about its own origin.</li>
 * </ul>
 *
 * <p>Bones are read flat and re-parented afterwards through a name map, so a
 * child may appear before its parent in the file. A bone naming a parent that
 * does not exist is dropped from the tree (it lands in neither
 * {@code model.bones} nor any child list) — legacy behaviour, preserved.</p>
 *
 * <p>Port note: legacy declared {@code throws net.minecraft.client.util.JsonException},
 * a client-only class that no longer exists in that form; the checked exception
 * is dropped, and {@link mchorse.chameleon.lib.ChameleonLoader#loadModel}
 * (the sole caller) already catches {@code Exception}.</p>
 *
 * Legacy source: chameleon/.../lib/parsing/ModelParser.java
 */
public class ModelParser
{
    public static Model parse(JsonObject object)
    {
