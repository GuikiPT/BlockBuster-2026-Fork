 ...
 *
 * <p>Verbatim port of Chameleon 1.2's own {@code lib/utils/MatrixStack} —
 * <b>not</b> vanilla's {@link net.minecraft.client.util.math.MatrixStack}, and
 * deliberately not replaced by it. The bone/cube helpers below encode
 * Chameleon's exact pivot-in / rotate / pivot-out order and its X-axis mirroring
 * (see {@link #translateBone}), which is what makes a {@code .geo.json} render
 * the same way Blockbench previews it. It stays on {@code javax.vecmath} like the
 * rest of the ported matrix math (P13); conversion to JOML happens only at the
 * render boundary in {@code ChameleonPostRenderer}.</p>
 *
 * <p>Lives in the common source set: it is pure math with no MC dependency, so
 * headless tests can exercise the bone transform chain.</p>
 *
 * Legacy source: chameleon/.../lib/utils/MatrixStack.java
 */
