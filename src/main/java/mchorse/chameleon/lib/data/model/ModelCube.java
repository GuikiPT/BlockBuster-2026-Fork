...
/**
 * One box of a {@link ModelBone}: its origin/size/pivot/rotation, per-face UVs,
 * and the six quads {@link #generateQuads} bakes out of them once at parse time.
 *
 * <p>Verbatim port of Chameleon 1.2's {@code ModelCube} — pure math, no MC
 * types. Two Bedrock quirks are load-bearing and preserved exactly:</p>
 * <ul>
 *   <li><b>Box UV uses floored sizes.</b> {@link #setupBoxUV} floors
 *       {@code size.x/y/z} before laying out the six faces, so a cube of
 *       fractional size still maps to the whole-pixel net Blockbench writes.</li>
 *   <li><b>The {@code down} face is authored upside down.</b> Its
 *       {@code tMaxY} is {@code boxUV.y}, not {@code tMinY + d} — the V range
 *       therefore runs backwards. That is what Bedrock does, and flipping it
 *       would mirror the bottom of every box model.</li>
 * </ul>
 *
 * Legacy source: chameleon/.../lib/data/model/ModelCube.java
 */
