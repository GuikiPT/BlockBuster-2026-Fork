...
/**
 * Keyframe interpolation modes, named by their <b>GeckoLib/Blockbench</b>
 * {@code easing} string ({@link #byName}) rather than by McLib's own names.
 *
 * <p>Two quirks are deliberate and must not be "fixed":</p>
 * <ul>
 *   <li>{@link #HERMITE} has no easing name — it is selected by
 *       {@code "lerp_mode": "catmullrom"} — and is the one mode that reads the
 *       neighbouring keyframes.</li>
 *   <li>The <b>elastic and bounce pairs are swapped</b> relative to McLib's
 *       {@link Interpolation} constants ({@code easeInElastic} maps to
 *       {@code ELASTIC_OUT}, and so on). That is how the GeckoLib plugin renders
 *       them, so matching the name to the "obviously correct" constant would make
 *       every exported animation play the wrong way round.</li>
 * </ul>
 *
 * <p>Port note: legacy's {@code EnumFacing.Axis} is
 * {@link net.minecraft.util.math.Direction.Axis} on 1.20.4.</p>
 *
 * Legacy source: chameleon/.../lib/data/animation/AnimationInterpolation.java
 */
public enum AnimationInterpolation
{
    ...
    {
        @Override
        public double interpolate(AnimationVector vector, MolangHelper.Component component, Direction.Axis axis, double factor)
        {
