...
/**
 * Abstract camera fixture (P171).
 *
 * Port notes: {@code duration} min is 1 (clamped in the ValueLong — a
 * 0-length fixture is unrepresentable); every subclass must keep a public
 * {@code (long)} constructor ({@link FixtureRegistry#fromType} reflects on
 * it); registration order name→color→duration→modifiers is the ByteBuf
 * contract; {@code color == 0} means "use the registry class color".
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/camera/fixtures/AbstractFixture.java
 */
