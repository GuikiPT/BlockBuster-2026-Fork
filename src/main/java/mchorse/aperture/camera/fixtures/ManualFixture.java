...
/**
 * Manual (recorded camera) fixture (P173).
 *
 * Port notes: {@code speed <= 0} sentinel yields a huge endTick (1e9), not
 * divide-by-zero; {@code setupRecorded} fills gap ticks with copies of the
 * last frame; playback picks the sub-tick frame by comparing {@code pt}
 * against the previous tick's last frame. The render-tick capture loop
 * itself is client GUI territory (P184).
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/camera/fixtures/ManualFixture.java
 */
public class ManualFixture extends AbstractFixture
{
     ...
     * @param partialTicks
     */
    public void recordFrame(PlayerEntity player, float partialTicks)
    {
        RenderFrame frame = new RenderFrame(player, partialTicks);

        frame.y += player.getStandingEyeHeight();
