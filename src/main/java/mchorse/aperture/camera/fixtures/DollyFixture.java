...
/**
 * Dolly fixture (P172) — extends IdleFixture, moves along the yaw/pitch
 * look vector by {@code distance} with a chosen interpolation. Verbatim
 * port ({@code Vec3d.scale} → yarn {@code multiply}).
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/camera/fixtures/DollyFixture.java
 */
public class DollyFixture extends IdleFixture
{
    ...
    public void applyFixture(long ticks, float partialTicks, float previewPartialTick, CameraProfile profile, Position pos)
    {
        ...
        Vec3d look = new Vec3d(sin * cos2, sin2, cos * cos2).normalize().multiply(this.distance.get());
