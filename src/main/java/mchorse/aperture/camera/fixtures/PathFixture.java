...
/**
 * Path camera fixture (P172).
 *
 * Port notes: speed channel is pre-seeded with keyframe {@code (0, 1)}; the
 * constant-speed solver is an <b>approximation</b> ported verbatim
 * (epsilon 0.00005, {@code factor *= -0.5} walk) — do not substitute an
 * exact arc-length solver; CIRCULAR mode interpolates polar (angle,
 * distance) around the (auto or manual) center with yaw normalization
 * crossing 180; {@code previewPartialTick} participates in path index math
 * (editor paused preview). Uses bundled javax.vecmath like legacy.
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/camera/fixtures/PathFixture.java
 */
