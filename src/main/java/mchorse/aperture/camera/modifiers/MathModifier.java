 ...
 * Math modifier (P176).
 *
 * Port notes: exact variable set
 * {@code t, o, pt, d, p, f, v, value, x, y, z, yaw, pitch, roll, fov};
 * velocity ({@code v}) is computed by evaluating the fixture/profile at
 * {@code offset+1} with modifiers, <b>excluding self</b> via the
 * {@code target} parameter (prevents infinite recursion); the same
 * expression is evaluated per active component with {@code value} swapped.
 * Verbatim.
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/camera/modifiers/MathModifier.java
 */
