...
/**
 * Verbatim P169 port — JSON array of typed objects; ByteBuf = int count
 * prefix + entries; sub-values renamed to <b>stringified indices</b> on
 * every mutation ({@link #sync()}) so {@code fixtures.3.duration}-style
 * undo paths resolve through {@code StructureBase.getProperty}. Malformed
 * entries yield null from the serializer and are skipped (total reader).
 * Legacy source:
 * .tools/legacy-src/aperture/src/main/java/mchorse/aperture/camera/values/ValueFixtures.java
 */
