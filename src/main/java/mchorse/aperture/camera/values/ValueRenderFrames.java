...
/**
 * Verbatim P169 port — JSON = array of per-tick arrays of RenderFrames;
 * ByteBuf = tick count, then per-tick frame count and 3 doubles + 5 floats
 * per frame (roll/fov/pt included). {@code getValue()} keeps the legacy
 * copy bug (adds the original list, not the copies).
 * Legacy source:
 * .tools/legacy-src/aperture/src/main/java/mchorse/aperture/camera/values/ValueRenderFrames.java
 */
