...
/**
 * Camera profile class (P170).
 *
 * Port notes: value registration order curves→modifiers→fixtures = the
 * top-level JSON key order; {@code setDirty(boolean)} <b>always</b> posts
 * {@link CameraProfileChangedEvent} (even for {@code false} — save-flow
 * icon flip); {@code UndoManager(30)} depth exactly; {@code copy()} keeps
 * the {@code NUMBERED_SUFFIX} auto-increment ({@code profile_2 →
 * profile_3}; no suffix → {@code _1}); {@code applyProfile} clamps
 * past-the-end progress to the last fixture at its <b>full duration</b>
 * (holds the final frame — Blockbuster longer-scene sync relies on it) and
 * applies profile-global modifiers only when the fixture-modifier loop
 * returned true (target short-circuit, P177).
 *
 * The legacy {@code @SideOnly applyCurves} delegation to
 * {@code ClientProxy.curveManager} is deferred with P180 (curves) — the
 * client runner owns that hook.
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/camera/CameraProfile.java
 */
public class CameraProfile extends StructureBase
{
    ...
    public void setDirty(boolean dirty)
    {
        ...
        CameraProfileChangedEvent.post(new CameraProfileChangedEvent(this));
    }
