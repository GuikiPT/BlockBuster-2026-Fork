...
/**
 * Label morph panel (roadmap P59.2) — the whole label morph in one scrolling
 * column: text, max width, anchor, colour + lighting, shadow (toggle, offset,
 * colour), background colour + offset, and the billboard toggle.
 *
 * <p>Port of Metamorph 1.4's {@code GuiLabelPanel}, widget for widget. The
 * trackpad tuning is legacy's and is deliberately uneven: {@code max} is an
 * integer trackpad limited to {@code -1..MAX_VALUE} stepping by 10 (−1 means
 * "no wrapping"), the anchors step by 0.01, the shadow offsets are clamped to
 * ±100 with 0.1/0.01/0.5 modifier values, and {@code offset} is left
 * unconstrained. Only the background colour picker edits alpha.</p>
 *
 * Legacy source: .tools/legacy-src/metamorph/src/main/java/mchorse/vanilla_pack/editors/panels/GuiLabelPanel.java
 */
public class GuiLabelPanel extends GuiMorphPanel<LabelMorph, GuiLabelMorph>
{
    ...
    public GuiLabelPanel(MinecraftClient mc, GuiLabelMorph editor)
    {
        super(mc, editor);
