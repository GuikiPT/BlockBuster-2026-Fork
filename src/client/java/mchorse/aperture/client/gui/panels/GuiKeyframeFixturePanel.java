...
/**
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/client/gui/panels/GuiKeyframeFixturePanel.java
 */
public class GuiKeyframeFixturePanel extends GuiAbstractFixturePanel<KeyframeFixture>
{
    ...
    public GuiKeyframeFixturePanel(MinecraftClient mc, GuiCameraEditor editor)
    {
        super(mc, editor);
        ...
        this.keys().register(IKey.lang("aperture.gui.panels.keys.toggle_keyframes"), LegacyKeyCodes.KEY_N, this::toggleKeyframes).category(CATEGORY);
    }
    ...
    public void editFixture(Position position)
    {
        ...
        /* Back-project the camera by the interpolated distance (+0.05
         * epsilon) along the view vector; the channel stores
         * distance - 0.05F — pairs with P173's +0.05F on apply */
    ...
    public void resize()
    {
        for (IGuiElement element : this.buttons.getChildren())
        {
            ...
            button.flex().w(GuiDraw.textWidth(this.font, button.label.get()) + 10);
        }
    ...
    public void draw(GuiContext context)
    {
        ...
        GuiDraw.drawStringWithShadow(this.font, this.title.get(), this.area.ex() - GuiDraw.textWidth(this.font, this.title.get()) - 10, this.graph.area.y - GuiDraw.fontHeight(this.font) - 5, 0xffffff);
