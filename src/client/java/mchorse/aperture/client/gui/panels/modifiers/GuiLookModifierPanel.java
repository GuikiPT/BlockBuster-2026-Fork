...
/**
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/client/gui/panels/modifiers/GuiLookModifierPanel.java
 *
 * Port note: 1.12.2 {@code RayTraceResult} → yarn {@code HitResult}
 * ({@code typeOfHit} → {@code getType()}, {@code getBlockPos()} via
 * {@code BlockHitResult}, {@code hitVec} → {@code getPos()}).
 */
public class GuiLookModifierPanel extends GuiAbstractModifierPanel<LookModifier>
{
    ...
    public GuiLookModifierPanel(MinecraftClient mc, LookModifier modifier, GuiModifiersManager modifiers)
    {
        ...
        {
            Point point = this.modifier.block.get().copy();

            point.x = value;
            this.modifiers.editor.postUndo(this.undo(this.modifier.block, point));
        });
        ...
        {
            this.modifiers.editor.postUndo(this.undo(this.modifier.atBlock, b.isToggled()));
            this.updateVisibility(true);
        });

        this.forward = new GuiToggleElement(mc, IKey.lang("aperture.gui.modifiers.panels.forward"), false, (b) ->
        {
            this.modifiers.editor.postUndo(this.undo(this.modifier.forward, b.isToggled()));
        });
    ...
    {
        HitResult result = center ? RayTracing.rayTrace(this.mc.player, 128, 0F) : RayTracing.rayTraceWithEntity(this.mc.player, 128);

        if (result != null)
        {
            if (center && result.getType() == HitResult.Type.BLOCK)
            {
                BlockPos pos = ((BlockHitResult) result).getBlockPos();

                this.modifiers.editor.postUndo(this.undo(this.modifier.block, new Point(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5)));
                this.fillData();
            }
            else if (!center && result.getType() != HitResult.Type.MISS)
            {
                Vec3d vec = result.getPos();

                this.modifiers.editor.postUndo(this.undo(this.modifier.block, new Point(vec.x, vec.y, vec.z)));
                this.fillData();
            }
        }
    }
