 ...
 *
 * <p>Port note: the 100 ms {@code updateSpeedCache} debounce keeps the legacy
 * shape — a timestamp armed in {@link #profileWasUpdated()} checked in
 * {@link #draw(GuiContext)} (frame-driven, no threads).</p>
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/client/gui/panels/GuiPathFixturePanel.java
 */
public class GuiPathFixturePanel extends GuiAbstractFixturePanel<PathFixture>
{
    ...
    public GuiPathFixturePanel(MinecraftClient mc, GuiCameraEditor editor)
    {
        super(mc, editor);
        ...
        this.keys().register(IKey.lang("aperture.gui.panels.keys.velocity"), LegacyKeyCodes.KEY_E, () -> this.useSpeed.clickItself(GuiBase.getCurrent())).held(LegacyKeyCodes.KEY_LSHIFT).active(editor::isFlightDisabled).category(CATEGORY);
    }
