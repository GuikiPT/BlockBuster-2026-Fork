 ...
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/client/gui/panels/modules/GuiPointsModule.java
 */
public class GuiPointsModule extends GuiAbstractModule
{
    ...
    private IKey pathPoints = IKey.lang("aperture.gui.panels.path_points");
    ...
    public GuiPointsModule(MinecraftClient mc, GuiCameraEditor editor, Consumer<Integer> picker)
    {
        super(mc, editor);
    ...
    public void moveForward()
    {
        ...
        /* LEGACY BUG (load-bearing): copy-paste from moveBack() — after
         * moving a point forward the selection jumps backward to
         * index - 1 instead of following the point to index + 1 */
        int nextIndex = this.index - 1;
    ...
    public void draw(GuiContext context)
    {
        ...
        GuiDraw.drawRect(x, y, x + this.scroll.w, y + this.scroll.h, ColorUtils.HALF_BLACK);
        ...
        for (int i = 0; i < c; i++)
        {
            ...
            int w = GuiDraw.textWidth(this.font, label);

            GuiDraw.drawRect(xx, y, xx + 20, y + 20, this.index == i ? 0xffcc1170 : 0xffff2280);
            GuiDraw.drawRect(xx + 19, y, xx + 20, y + 20, 0x22000000);
            GuiDraw.drawStringWithShadow(this.font, label, xx + 10 - w / 2, y + 6, 0xffffff);
        }
        ...
        if (scroll != 0)
        {
            ...
            GuiDraw.drawRect(bx, by, bx + scroll, by + 2, ColorUtils.HALF_BLACK);
        }
        ...
        String label = this.pathPoints.get();
        int w = GuiDraw.textWidth(this.font, label);

        GuiDraw.drawStringWithShadow(this.font, label, this.scroll.x + this.scroll.w / 2 - w / 2, this.scroll.y - 14, 0xffffff);
    }
}
