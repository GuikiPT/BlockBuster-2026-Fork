...
/**
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/client/gui/panels/GuiDollyFixturePanel.java
 */
public class GuiDollyFixturePanel extends GuiAbstractFixturePanel<DollyFixture>
{
    ...
    public GuiDollyFixturePanel(MinecraftClient mc, GuiCameraEditor editor)
    {
        super(mc, editor);
    ...
    /**
     * Live speed label math (extracted for testability):
     * {@code distance / (duration / 20)} blocks per second.
     */
    public static double calculateSpeed(DollyFixture fixture)
    {
        return fixture.distance.get() / (fixture.getDuration() / 20D);
    }

    @Override
    public void draw(GuiContext context)
    {
        double speed = calculateSpeed(this.fixture);
        String label = IKey.format("aperture.gui.panels.dolly.speed", GuiTrackpadElement.FORMAT.format(speed)).get();

        GuiDraw.drawTextBackground(this.font, label, this.area.mx(GuiDraw.textWidth(this.font, label)), this.area.ey() - GuiDraw.fontHeight(this.font) - 20, 0xffffff, ColorUtils.HALF_BLACK);
