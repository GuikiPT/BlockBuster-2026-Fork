...
/**
 * Base class for a camera modifier panel (P185).
 *
 * <p>Header buttons (remove, lock/unlock = enabled, move up/down, copy,
 * envelope) are visible only on hover; an enabled panel is tinted with its
 * registry color, a disabled panel is rendered with the {@code DISABLED} icon
 * pattern. The title tooltip uses the {@code "<key>_description"} lang
 * convention.</p>
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/client/gui/panels/modifiers/GuiAbstractModifierPanel.java
 */
public abstract class GuiAbstractModifierPanel<T extends AbstractModifier> extends GuiElement
{
    ...
    public GuiAbstractModifierPanel(MinecraftClient mc, T modifier, GuiModifiersManager modifiers)
    {
        super(mc);
    ...
    public void draw(GuiContext context)
    {
        ...
        {
            GuiDraw.bindColor(ColorUtils.HALF_BLACK + this.color);
