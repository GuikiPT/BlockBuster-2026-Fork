...
/**
 * Camera modifiers manager (P185).
 *
 * <p>Edits global (profile) modifiers when no fixture is selected, else the
 * selected fixture's modifiers (the title switches). The add palette lists the
 * {@link ModifierRegistry} types as colored buttons; copy/paste uses deep
 * copies; move up/down reorders. Every mutation posts an unmergeable
 * list-snapshot undo that restores the panel scroll.</p>
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/client/gui/GuiModifiersManager.java
 */
public class GuiModifiersManager extends GuiElement
{
    ...
    public GuiModifiersManager(MinecraftClient mc, GuiCameraEditor editor)
    {
        super(mc);
    ...
    public void addModifierPanel(AbstractModifier modifier)
    {
        ...
        if (clazz != null)
        {
            try
            {
                GuiAbstractModifierPanel<AbstractModifier> panel = (GuiAbstractModifierPanel<AbstractModifier>) clazz.getConstructor(MinecraftClient.class, modifier.getClass(), GuiModifiersManager.class).newInstance(this.mc, modifier, this);
    ...
    public void draw(GuiContext context)
    {
        ...
        GuiDraw.drawRect(this.area.x, this.area.y, this.area.ex(), this.area.y + h, 0xaa000000);
