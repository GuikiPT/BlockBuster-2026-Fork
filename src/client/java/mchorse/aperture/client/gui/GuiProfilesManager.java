...
/**
 * Camera profile manager GUI (P183)
 *
 ...
 * Port notes:
 * <ul>
 * <li>P182 camera networking: when connected to an Aperture server as OP,
 * {@code init()} sends {@code PacketRequestCameraProfiles}; the server reply
 * ({@code PacketCameraProfileList}) merges client + server names via
 * {@code ClientHandlerCameraProfileList}. Otherwise only client profiles are
 * listed (the {@code ClientProxy.server} flag is set by the P182
 * {@code PacketAperture} handshake).</li>
 * <li>{@code GuiCurves} (the profile keyframes editor) landed with the P185
 * remainder — the keyframes button toggles the profile list out for the curves
 * editor, exactly like 1.8.2.</li>
 * <li>OP check: legacy {@code OpHelper.isPlayerOp()} (client) — the ported
 * P21 client half of {@code OpHelper}, called directly like legacy.</li>
 * </ul>
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/client/gui/GuiProfilesManager.java
 */
public class GuiProfilesManager extends GuiElement
{
    ...
    public GuiProfilesManager(MinecraftClient mc, GuiCameraEditor editor)
    {
        super(mc);
    ...
    private void add(String name)
    {
        ...
        AbstractDestination destination = AbstractDestination.create(name);

        this.addProfile(destination, true);

        /* Commit the empty profile immediately, while the connection is
         * certainly alive. Legacy created it in memory only, so a profile that
         * was never manually saved had no file behind it at all — and the one
         * moment the auto-save pass runs (disconnect) is the moment a
         * ServerDestination can no longer reach the server, so it could not
         * create that file either. Naming a profile now means it exists. */
        CameraProfile profile = this.profiles.getBy(destination);
    ...
    public void init()
    {
        ...
        {
            /* P182: ask the Aperture server for its profile list; the reply
             * (PacketCameraProfileList) merges both client + server names into
             * this manager via ClientHandlerCameraProfileList. */
    ...
    {
        public GuiCameraProfilesSearchList(MinecraftClient mc, Consumer<List<CameraProfile>> callback)
        {
            super(mc, callback);
        }

        @Override
        protected GuiListElement<CameraProfile> createList(MinecraftClient mc, Consumer<List<CameraProfile>> callback)
        {
    ...
    {
        public GuiCameraProfilesList(MinecraftClient mc, Consumer<List<CameraProfile>> callback)
        {
            super(mc, callback);
        }
        ...
        protected void drawElementPart(CameraProfile element, int i, int x, int y, boolean hover, boolean selected)
        {
            ...
            /* Legacy GlStateManager.color: non-loaded profiles draw grey */
            GuiDraw.bindColor(hasProfile ? 0xffffffff : 0xff808080);

            (element.getDestination() instanceof ClientDestination ? Icons.FOLDER : Icons.SERVER).render(x + 2, y + 2);

            GuiDraw.resetColor();

            GuiDraw.drawStringWithShadow(this.font, element.getDestination().getFilename(), x + 4 + 16, y + 6, hasProfile ? (hover ? 16777120 : 0xffffff) : 0x888888);
        }
