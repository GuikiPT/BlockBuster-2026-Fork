...
/**
 * GUI playback timeline (P183)
 *
 * This class is responsible for rendering and controlling the playback.
 *
 * Port notes:
 * <ul>
 * <li>{@code Minecraft.IS_RUNNING_ON_MAC} → {@code MinecraftClient.IS_SYSTEM_MAC}
 * (natural-scrolling compensation branch kept verbatim — the GuiBase bridge
 * already restores legacy 1.12.2 wheel magnitudes/signs).</li>
 * <li>Pixel constants are UX parity: ±5 px edge-resize picking with the
 * 6 px drag dead zone before resizing engages.</li>
 * <li>Wheel zoom clamps 0.001–1000.</li>
 * </ul>
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/client/gui/GuiPlaybackScrub.java
 */
public class GuiPlaybackScrub extends GuiElement
{
    ...
    public GuiPlaybackScrub(MinecraftClient mc, GuiCameraEditor editor, CameraProfile profile)
    {
        super(mc);
    ...
    public void draw(GuiContext context)
    {
        ...
        /* Visual duration resize (6 px dead zone before the drag engages) */
        ...
        if (this.profile != null)
        {
            ...
            int width = GuiDraw.textWidth(this.font, label) + 4;
            ...
            for (int i = 0; i < this.profile.fixtures.size(); i++)
            {
                ...
                if (fixture instanceof ManualFixture)
                {
                    ...
                    if (end < rightMargin)
                    {
                        GuiDraw.drawRect(end + 1, y + (selected ? 12 : 15), rightMargin, y + h - 1, selected ? 0xaa000000 : 0x66000000);
                    }

                    if (end > leftMargin)
                    {
                        GuiDraw.drawRect(start + 1, y + (selected ? 12 : 15), end + 1, y + h - 1, (selected ? 0xdd000000 : 0x66000000) + color);
                    }

                    if (start > leftMargin)
                    {
                        GuiDraw.drawRect(leftMargin + 1, y + (selected ? 12 : 15), start + 1, y + h - 1, selected ? 0xaa000000 : 0x66000000);
                    }
                }
                else
                {
                    GuiDraw.drawRect(leftMargin + 1, y + (selected ? 12 : 15), rightMargin, y + h - 1, (selected ? 0xdd000000 : 0x66000000) + color);
                }

                GuiDraw.drawRect(rightMargin, y + 1, rightMargin + 1, y + h - 1, 0xff000000 + color);

                if (selected)
                {
                    GuiDraw.drawVerticalGradientRect(leftMargin + 1, y + 5, rightMargin, y + 12, 0x000088ff, 0x880088ff);
                    GuiDraw.drawVerticalGradientRect(leftMargin + 1, y + 15, rightMargin, y + h - 1, 0, 0x44000000);
                }
                ...
                if (fixture instanceof PathFixture)
                {
                    ...
                    if (c > 1)
                    {
                        ...
                        for (int j = 1; j < c; j++)
                        {
                            ...
                            GuiDraw.drawRect(px, y + 5, px + 1, y + h - 1, highlight);
                        }
                    }
                }
                ...
                if (!name.isEmpty())
                {
                    int lw = GuiDraw.textWidth(this.font, name);
                    ...
                    while (lw + 10 >= rightMargin - leftMargin && !name.isEmpty())
                    {
                        ...
                        lw = GuiDraw.textWidth(this.font, name);
                        shortened = true;
                    }

                    if (!name.isEmpty())
                    {
                        GuiDraw.drawStringWithShadow(this.font, shortened ? name + "..." : name, leftMargin + 4, y + 6, textColor);
                    }
                }
            ...
            if (this.editor.creating)
            {
                for (Integer marker : this.editor.markers)
                {
                    ...
                    GuiDraw.drawRect(mx, y + 5, mx + 1, y + h - 1, 0xaaff0000);
                }
            }
            ...
            if (this.loopMin != this.loopMax)
            {
                ...
                if (maxX >= this.area.x + 1 && minX < this.area.ex() - 1)
                {
                    ...
                    GuiDraw.drawRect(minX, this.area.y, minX + 1, this.area.ey(), 0xff88ffff);
                    GuiDraw.drawRect(maxX - 1, this.area.y, maxX, this.area.ey(), 0xff88ffff);
                }
            }
            ...
            {
                GuiDraw.drawVerticalGradientRect(stopX + 1, y + h / 2, x + w - 1, y + h, 0, ColorUtils.HALF_BLACK);
                GuiDraw.drawRect(stopX, y + h / 2, stopX + 1, y + h, 0xaaffffff);
            }
            ...
            /* Draw the marker */
            GuiDraw.drawRect(tx, y + 1, tx + 2, y + h - 1, 0xff57f52a);
            ...
            int ow = GuiDraw.textWidth(this.font, offsetLabel);
            int fontHeight = GuiDraw.fontHeight(this.font);

            GuiDraw.drawStringWithShadow(this.font, offsetLabel, tx - ow / 2 + 1, y + h - fontHeight * 3 - 1, 0xffffff);
            ...
            /* Draw the tick label */
            GuiDraw.drawRect(tx + 2, y + h - 3 - fontHeight, tx + 2 + width, y + h - 1, 0xaa57f52a);
            GuiDraw.drawStringWithShadow(this.font, label, tx + 4, y + h - fontHeight - 1, 0xffffff);
            ...
            if (drawnMarker)
            {
                ...
                GuiDraw.drawRect(markerOffset - 4, this.area.y - 1, markerOffset + 5, this.area.y, 0xaaffffff);
                GuiDraw.drawRect(markerOffset - 5, this.area.y - 1 - 2, markerOffset - 4, this.area.y + 2, 0xaaffffff);
                GuiDraw.drawRect(markerOffset + 5, this.area.y - 1 - 2, markerOffset + 6, this.area.y + 2, 0xaaffffff);
            }
        }

        /* Draw background */
        GuiDraw.drawRect(x + 1, y + h / 2, x + 2, y + h, 0xaaffffff);
        GuiDraw.drawRect(x + w - 2, y + h / 2, x + w - 1, y + h, 0xaaffffff);
        GuiDraw.drawRect(x, y + h - 1, x + w, y + h, 0xffffffff);
    }
    ...
    private void drawTickMarks(int y, int h)
    {
        ...
        for (int j = start; j <= end; j += mult)
        {
            ...
            GuiDraw.drawTextBackground(this.font, value, xx - GuiDraw.textWidth(this.font, value) / 2, y, 0xffffff, ColorUtils.HALF_BLACK, 2);
            GuiDraw.drawRect(xx, y + h / 2, xx + 1, y + h, 0x66ffffff);
        }
    }
}
