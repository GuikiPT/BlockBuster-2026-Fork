 ...
 *
 * <p>P184 note: the plan files this wrapper family under P185, but the
 * keyframe fixture panel (and the path panel's speed graph) need it — the
 * full class is ported here, including the 400 ms undo-merge debounce
 * ({@link #markUndo(int)}/{@code submitUndo}).</p>
 *
 * Legacy source: .tools/legacy-src/aperture/src/main/java/mchorse/aperture/client/gui/utils/GuiCameraEditorKeyframesEditor.java
 */
public abstract class GuiCameraEditorKeyframesEditor<E extends GuiKeyframeElement> extends GuiKeyframesEditor<E>
{
    ...
    public GuiCameraEditorKeyframesEditor(MinecraftClient mc, GuiCameraEditor editor)
    {
        super(mc);

        this.editor = editor;

        this.interp.keys().register(IKey.lang("aperture.gui.panels.keys.graph_interp"), LegacyKeyCodes.KEY_LBRACKET, this::toggleInterpolation).held(LegacyKeyCodes.KEY_LCONTROL).category(GuiAbstractFixturePanel.CATEGORY).active(editor::isFlightDisabled);
        this.easing.keys().register(IKey.lang("aperture.gui.panels.keys.graph_easing"), LegacyKeyCodes.KEY_RBRACKET, this::toggleEasing).held(LegacyKeyCodes.KEY_LCONTROL).category(GuiAbstractFixturePanel.CATEGORY).active(editor::isFlightDisabled);
    }
