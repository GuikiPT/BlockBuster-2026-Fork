...
/**
 * Port of Blockbuster 2.7.2's {@code audio/AudioLibrary} (roadmap P188).
 *
 * <p>Scans {@code config/blockbuster/audio} for {@code *.wav}, lazy-loads them
 * into {@link AudioFile}s (player + waveform + mtime), executes the per-state
 * {@link WavePlayer} call sequences for incoming audio commands, hot-reloads
 * edited files by mtime, and never crashes on bad input (a corrupt file caches
 * an empty placeholder). Client-side class — it references AL/GL types via
 * {@link WavePlayer}/{@link Waveform}, exactly as {@code @SideOnly(CLIENT)}
 * implied in 1.12 (the class had no annotation but was only ever touched from
 * {@code ClientProxy}).</p>
 *
 * <p>Total-reader chain preserved verbatim: {@code WaveReader.read} returns
 * {@code null} on a parse failure, which NPEs inside {@link #load}'s try and is
 * caught into the empty placeholder — behavioral outcome identical to a
 * pre-null-check but keeping the {@code printStackTrace} + cache.</p>
 */
public class AudioLibrary
{
    ...
    private AudioFile load(String name, File file)
    {
        ...
        try
        {
            ...
            WavePlayer player = this.initializePlayer(wave);
    ...
    /**
     * Seam for the {@code WavePlayer} construction path (P188/P192). The
     * production implementation constructs a real AL-backed player; headless
     * tests subclass {@code AudioLibrary} and return a no-AL fake so
     * {@link #load} exercises the total-reader + caching logic without an AL
     * device.
     */
    protected WavePlayer initializePlayer(Wave wave)
    {
        return new WavePlayer().initialize(wave);
    }
    ...
    /**
     * Per-state {@link WavePlayer} call sequences. Legacy visibility was
     * {@code private}; widened to package-private so headless state-machine
     * tests can drive it directly with a fake player (P188/P192 seam). Order of
     * calls within each case is load-bearing.
     */
    void handleAudioState(AudioState state, WavePlayer player, float seconds, float elapsedDelay)
    {
        switch (state)
        {
            case REWIND:
