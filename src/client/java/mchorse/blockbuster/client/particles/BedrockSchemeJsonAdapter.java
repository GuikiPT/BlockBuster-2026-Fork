...
public class BedrockSchemeJsonAdapter implements JsonDeserializer<BedrockScheme>, JsonSerializer<BedrockScheme>
{
    ...
    private void parseComponents(BedrockScheme scheme, JsonObject components) throws MolangException
    {
        for (Map.Entry<String, JsonElement> entry : components.entrySet())
        {
            ...
            if (this.components.containsKey(key))
            {
                ...
                {
                    /* Total reader: reflection failure logs and continues (legacy printed to stdout) */
                    Blockbuster.LOGGER.warn("Failed to parse given component {} in {}!", key, scheme.identifier);
                }
            }
            else
            {
                /*
                 * Total reader: unknown/unsupported component keys are dropped
                 * (matching 1.12.2, which simply loses them on resave). The
                 * warning does not alter output files.
                 */
                Blockbuster.LOGGER.warn("Skipping unknown particle component {} in {}", key, scheme.identifier);
            }
        }
    }
    ...
    {
        /* Legacy's && evaluated get(key) on a missing key and NPEd before the
         * descriptive message could throw */
