...
/**
 * Particle tint color — solid (possibly MoLang-driven) or gradient.
 *
 * <p>Ported verbatim from the 1.12.2 legacy engine
 * ({@code mchorse.blockbuster.client.particles.components.appearance.Tint}).
 * Golden-visible quirks preserved exactly:</p>
 * <ul>
 *   <li>Hex is <b>alpha-first</b> ({@code #AARRGGBB}); the &gt;6-digit
 *       {@code Integer.parseInt} workaround splits alpha off separately.</li>
 *   <li>{@code Solid.toJson} emits an <b>empty array</b> when all four channels
 *       are the constant 1 (so the tinting component drops it on resave).</li>
 *   <li>{@code toHexJson} writes the {@code #AA} alpha prefix only when
 *       {@code a < 255}.</li>
 *   <li>Gradient stops are normalized by the max stop on parse and re-multiplied
 *       on write ({@link Gradient.ColorStop#getStop(float)}), so a round-trip
 *       preserves the original stop keys.</li>
 *   <li>Parse failure silently keeps white.</li>
 * </ul>
 */
