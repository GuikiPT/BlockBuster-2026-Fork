...
/**
 * {@code minecraft:emitter_shape_disc} (roadmap P151).
 *
 * <p>Extends the sphere with a {@code plane_normal} (the string {@code "x"}/
 * {@code "y"}/{@code "z"} or a 3-element MoLang vector). A random in-plane point
 * is rotated by a quaternion built as {@code (nx, ny, nz, 1)} — <b>non-normalized
 * with w=1, an inherited quirk that skews the disc plane</b>; do not "fix" it.
 * Unlike every other shape, the disc <b>adds</b> its offset to
 * {@code particle.position} ({@code +=}) instead of assigning, so it composes
 * with a prior point-offset — authored effects depend on this.</p>
 */
public class BedrockComponentShapeDisc extends BedrockComponentShapeSphere
{
    ...
    public void apply(BedrockEmitter emitter, BedrockParticle particle)
    {
        ...
        Vector4f position = new Vector4f((float) RandomProvider.nextDouble() - 0.5F, 0, (float) RandomProvider.nextDouble() - 0.5F, 0);
        position.normalize();
        rotation.transform(position);

        position.scale((float) (this.radius.get() * (this.surface ? 1 : RandomProvider.nextDouble())));
