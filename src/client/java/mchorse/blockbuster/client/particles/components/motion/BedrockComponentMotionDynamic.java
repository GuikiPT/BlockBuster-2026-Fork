...
/**
 * {@code minecraft:particle_motion_dynamic} (roadmap P151).
 *
 * <p>Keys {@code linear_acceleration} {@code [x,y,z]}, {@code linear_drag_coefficient},
 * {@code rotation_acceleration} and {@code rotation_drag_coefficient}. Accelerations
 * are accumulated onto the particle each update (the rotation acceleration is
 * divided by {@code 20} here at accumulate time — the spin component's {@code /20}
 * is at init, keep them separate); drag values overwrite the particle's fields
 * each tick. {@code linear_acceleration} is <b>always</b> serialized, even when
 * all zero (golden-visible), unlike the other keys which are omitted when zero.</p>
 */
