...
/**
 * {@code minecraft:emitter_rate_steady} — roadmap P150.
 *
 * <p>Spawns particles continuously. Crucially, <b>spawning happens in the render
 * pass ({@link #postRender})</b>, not the tick, using partialTicks-precise
 * emitter age and fractional {@code spawnedParticles} accounting — so a paused
 * render pass stops emission. Particles above {@code max_particles} are subtracted
 * from the accounting so they respawn once room frees ({@link BedrockEmitter}
 * special-cases this component in its render path to avoid double-spawning in the
 * morph pass — see P153).</p>
 *
 * <p>Keys: {@code spawn_rate} (default {@code 1}, omitted when
 * {@link MolangExpression#isOne}); {@code max_particles} (default constant
 * {@code 50}, stored in {@code particles}, omitted when {@code 50}).
 * {@link #getSortingIndex()} is {@code 10}.</p>
 */
