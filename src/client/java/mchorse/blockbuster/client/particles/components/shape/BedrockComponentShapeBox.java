...
/**
 * {@code minecraft:emitter_shape_box} (roadmap P151).
 *
 * <p>Spawns particles uniformly inside a box of {@code half_dimensions} centered
 * on {@code offset}. When {@code surface_only}, one random axis is snapped to a
 * face via {@code roll = (random * 600) % 6}.</p>
 */
public class BedrockComponentShapeBox extends BedrockComponentShapeBase
{
    ...
    public void apply(BedrockEmitter emitter, BedrockParticle particle)
    {
        ...
        particle.position.x = centerX + ((float) RandomProvider.nextDouble() * 2 - 1F) * w;
        particle.position.y = centerY + ((float) RandomProvider.nextDouble() * 2 - 1F) * h;
        particle.position.z = centerZ + ((float) RandomProvider.nextDouble() * 2 - 1F) * d;
