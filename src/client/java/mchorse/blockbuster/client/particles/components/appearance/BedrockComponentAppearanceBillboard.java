...
/**
 * Billboard appearance component — {@code minecraft:particle_appearance_billboard}.
 *
 * <p>Ported operation-for-operation from the 1.12.2 legacy engine
 * ({@code mchorse.blockbuster.client.particles.components.appearance.BedrockComponentAppearanceBillboard}).
 * The facing/UV/flipbook math is load-bearing (see S13 P152 quirks); do not "fix"
 * the apparent oddities:</p>
 * <ul>
 *   <li>Quad size = {@code sizeW/H * 2.25} (magic constant); {@code renderOnScreen}
 *       uses a fixed 0.5 size scaled by {@code scale * 2.75}.</li>
 *   <li>{@code texture_width}/{@code texture_height} default <b>128</b> and are
 *       public/mutable (the editor writes live GL texture dimensions into them).</li>
 *   <li>Perspective sign flips: {@code perspective == 2} (front view) negates
 *       width; {@code perspective == 100} (GUI sentinel) mirrors yaw and negates
 *       both width and height (only for non-lookAt modes).</li>
 *   <li>Flipbook stretch divides by {@code expirationDelay} when the particle has
 *       an expireAge set (Blockbuster extension).</li>
 * </ul>
 *
 * <p>Vertex emission targets the P153 render pipeline: the legacy
 * {@code POSITION_TEX_LMAP_COLOR} format maps to yarn's
 * {@link VertexFormats#POSITION_TEXTURE_COLOR_LIGHT} — the emitter's
 * {@code renderParticles} (P153) opens the buffer with that format, and this
 * component emits vertices in position/texture/color/light order to match. The
 * order is the format's, not a preference: {@code BufferBuilder}'s fluent
 * setters fill the <i>next</i> element, so a permuted-but-complete chain writes
 * the colour bytes into the lightmap element and nothing throws
 * ({@code VertexEmissionOrderTest} is the tree-wide guard).</p>
 */
public class BedrockComponentAppearanceBillboard extends BedrockComponentBase implements IComponentParticleRender
{
    ...
    public void render(BedrockEmitter emitter, BedrockParticle particle, BufferBuilder builder, float partialTicks)
    {
        ...
        this.translateQuad(emitter, px, py, pz);
        ...
        builder.vertex(this.vertices[0].x, this.vertices[0].y, this.vertices[0].z).texture(u1, v1).color(particle.r, particle.g, particle.b, particle.a).light(light).next();
        builder.vertex(this.vertices[1].x, this.vertices[1].y, this.vertices[1].z).texture(u2, v1).color(particle.r, particle.g, particle.b, particle.a).light(light).next();
        builder.vertex(this.vertices[2].x, this.vertices[2].y, this.vertices[2].z).texture(u2, v2).color(particle.r, particle.g, particle.b, particle.a).light(light).next();
        builder.vertex(this.vertices[3].x, this.vertices[3].y, this.vertices[3].z).texture(u1, v2).color(particle.r, particle.g, particle.b, particle.a).light(light).next();
    }

    /**
     * Place the finished quad at the particle's world position, expressed
     * relative to the emitter's render origin — roadmap P275.
     *
     * <p>This is the last surviving statement of legacy
     * {@code BufferBuilder.setTranslation(-camX, -camY, -camZ)}: 1.12.2 offset
     * the vertices as they were written into the buffer, and the buffer no
     * longer offers that on 1.20.4, so the offset is applied here instead. It
     * matters that it is applied to the <b>vertex data</b> and not to the
     * model-view matrix: vanilla's {@code particle.vsh} derives its fog distance
     * from the raw {@code Position} attribute, decomposed per-axis for the
     * cylinder fog shape the world pass uses, and world-absolute positions there
     * push every quad past {@code FogEnd} — it draws as a flat sheet of fog
     * (sky) colour with no texture at all. See
     * {@link BedrockEmitter#setupOpenGL(float)} for the full account.</p>
     *
     * <p>Both quad-emitting appearance components route through here so the two
     * cannot drift; the emitter's own origin is zero outside the world pass, so
     * the dashboard viewport and the GUI thumbnail are byte-identical to what
     * they drew before. Subtracting in {@code double} <i>before</i> the
     * {@code float} cast is also what keeps a far-from-origin world free of
     * quad-sized positional jitter.</p>
     */
    protected void translateQuad(BedrockEmitter emitter, double px, double py, double pz)
    {
        this.transform.setTranslation(new Vector3f(
            (float) (px - emitter.renderOriginX),
            (float) (py - emitter.renderOriginY),
            (float) (pz - emitter.renderOriginZ)));
    }
    ...
    public void renderOnScreen(BedrockParticle particle, int x, int y, float scale, float partialTicks)
    {
        ...
        /* SEAM(P153): GUI preview GL state (texture bind, blend, projection) is
         * owned by P153's renderOnScreen preview path. The vertex/UV math here is
         * final; the caller sets up the shader texture before invoking. */
        ...
        builder.vertex(this.vertices[0].x, this.vertices[0].y, this.vertices[0].z).texture(u1, v1).color(particle.r, particle.g, particle.b, particle.a).next();
        builder.vertex(this.vertices[1].x, this.vertices[1].y, this.vertices[1].z).texture(u2, v1).color(particle.r, particle.g, particle.b, particle.a).next();
        builder.vertex(this.vertices[2].x, this.vertices[2].y, this.vertices[2].z).texture(u2, v2).color(particle.r, particle.g, particle.b, particle.a).next();
        builder.vertex(this.vertices[3].x, this.vertices[3].y, this.vertices[3].z).texture(u1, v2).color(particle.r, particle.g, particle.b, particle.a).next();
