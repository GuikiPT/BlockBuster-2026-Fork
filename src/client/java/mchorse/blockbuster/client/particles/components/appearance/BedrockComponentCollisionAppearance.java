...
/**
 * Collision appearance component — {@code blockbuster:particle_collision_appearance}.
 *
 * <p>Ported verbatim from the 1.12.2 legacy engine. After the particle's first
 * intersection it switches to an alternate texture/material/UV. It inherits all
 * billboard fields, adds {@code enabled} (MoLang), {@code lit} (bool),
 * {@code material}, {@code texture}, and renders at <b>sortingIndex 200</b>. The
 * fullbright value is {@code 15728880} when {@code lit}. The complex lit-swap dance
 * between {@code component.lit} and {@code emitter.lit} (depending on whether
 * collision tinting is also active) is load-bearing — do not simplify it.</p>
 *
 * <p>Its {@code calculateUVs} override differs from the billboard's only in the
 * flipbook-stretch denominator: {@code lifetime - firstIntersection} (the
 * collision variant of the Blockbuster expiration-stretch extension).</p>
 */
public class BedrockComponentCollisionAppearance extends BedrockComponentAppearanceBillboard implements IComponentParticleRender
{
    ...
    public void render(BedrockEmitter emitter, BedrockParticle particle, BufferBuilder builder, float partialTicks)
    {
        ...
        /* P275: the same camera-relative placement the base pass uses — see
         * BedrockComponentAppearanceBillboard.translateQuad. */
