...
/**
 * Port of Blockbuster 2.7.2's abstract {@code GuiSnowstormSection} (roadmap
 * P141) — the shell↔section contract for the Snowstorm particle editor.
 *
 * <p>Legacy source: blockbuster-1.12/src/main/java/mchorse/blockbuster/client/gui/dashboard/panels/snowstorm/sections/GuiSnowstormSection.java</p>
 *
 * <p><b>Shell phase (P141):</b> only this abstract base ships now. The 13
 * registered concrete sections (+ the commented-out particle-morph one) land in
 * S13/P155 and slot into {@link GuiSnowstorm#addSection(GuiSnowstormSection)}
 * in the frozen order recorded on {@link GuiSnowstorm#SECTION_ORDER}. Each
 * section is a collapsible header (title bar) over a {@link #fields} column; the
 * collapsed/expanded state lives in the static {@link GuiSectionManager} keyed
 * by the concrete class' simple name, so it persists across scheme switches and
 * dashboard rebuilds.</p>
 */
public abstract class GuiSnowstormSection extends GuiElement
{
    ...
    public GuiSnowstormSection(MinecraftClient mc, GuiSnowstorm parent)
    {
        super(mc);
