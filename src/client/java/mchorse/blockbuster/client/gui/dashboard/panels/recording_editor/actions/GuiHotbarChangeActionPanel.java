...
/**
 * P139 — hotbar-slot content editor: a slot trackpad (0..8), an item picker,
 * and a durability trackpad that only appears when the picked item is
 * damageable.
 *
 * <p>Legacy quirks reproduced:</p>
 * <ul>
 * <li>The durability trackpad's value is the <b>remaining-durability percent</b>
 * (100% = pristine); it is back-converted to
 * {@code damage = (1 - pct/100) * maxDamage} on a copied stack.</li>
 * <li>The durability field is added/removed dynamically ({@link #updateFields})
 * with a parent {@code resize()}.</li>
 * </ul>
 */
public class GuiHotbarChangeActionPanel extends GuiActionPanel<HotbarChangeAction>
{
    ...
    public GuiHotbarChangeActionPanel(MinecraftClient mc, GuiRecordingEditorPanel panel)
    {
        super(mc, panel);
        ...
        this.durability = new GuiTrackpadElement(mc, (b) ->
        {
            if (this.item.getStack().getMaxDamage() != 0)
            {
                ...
                newItemStack.setDamage((int) ((1 - b.floatValue() / 100F) * this.item.getStack().getMaxDamage()));
    ...
    protected void updateFields()
    {
        ...
        double durability = (this.item.getStack().getMaxDamage() == 0) ? 1 : (1 - (double) this.item.getStack().getDamage() / (double) this.item.getStack().getMaxDamage());
