...
/**
 * Port of Blockbuster 2.7.2's {@code GuiSnowstormExpirationSection} (roadmap
 * P155).
 *
 * <p>Legacy source: blockbuster-1.12/src/main/java/mchorse/blockbuster/client/gui/dashboard/panels/snowstorm/sections/GuiSnowstormExpirationSection.java</p>
 *
 * <p><b>Parity quirks:</b> block lists only compile into the component in
 * {@link #beforeSave}; empty slots serialize as {@code Blocks.AIR}; the
 * context-menu label is {@code IKey.lang("Remove block")} — a literal string
 * passed as a lang key that renders untranslated — preserved. 1.12's
 * {@code ItemBlock}/{@code net.minecraft.init.Blocks} become yarn
 * {@link BlockItem}/{@link Blocks}.</p>
 */
public class GuiSnowstormExpirationSection extends GuiSnowstormSection
{
    ...
    public GuiSnowstormExpirationSection(MinecraftClient mc, GuiSnowstorm parent)
    {
        super(mc, parent);
    ...
    private void compileBlocks(BedrockComponentExpireBlocks component, GuiBlocksSection section)
    {
        ...
        for (IGuiElement child : section.blocks.getChildren())
        {
            if (child instanceof GuiSlotElement)
            {
                ...
                {
                    component.blocks.add(((BlockItem) slot.getStack().getItem()).getBlock());
                }
    ...
    public static class GuiBlocksSection extends GuiElement
    {
        ...
        public GuiBlocksSection(MinecraftClient mc, IKey title, GuiSnowstormExpirationSection parent)
        {
            super(mc);
