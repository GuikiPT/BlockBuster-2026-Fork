...
/**
 * Port of Blockbuster 2.7.2's {@code GuiGradientEditor} (roadmap P155) — the
 * 20px gradient strip + color-stop handle editor used by the lighting section.
 *
 * <p>Legacy source: blockbuster-1.12/src/main/java/mchorse/blockbuster/client/gui/dashboard/panels/snowstorm/utils/GuiGradientEditor.java</p>
 *
 * <p><b>Parity quirks:</b> drag starts only after a 3px mouse-move threshold;
 * {@link #setGradient} auto-inserts a stop at 0 when empty; new stops default to
 * opaque white constants; the context menu offers "remove stop" when stops &gt; 1
 * but {@link #removeColorStop} only acts when stops &gt; 2 — with exactly 2 stops
 * the item silently no-ops. All preserved. Legacy immediate-mode
 * {@code Gui.drawRect}/{@code GlStateManager.color} become
 * {@link GuiDraw#drawRect} and the self-colored {@code Icons.CHECKBOARD}.</p>
 */
public class GuiGradientEditor extends GuiElement
{
    ...
    public GuiGradientEditor(MinecraftClient mc, GuiSnowstormSection section, GuiColorElement color)
    {
        super(mc);
    ...
    public void draw(GuiContext context)
    {
        ...
        if (first.stop > 0)
        {
            ...
            GuiDraw.drawRect(this.a.x, this.a.y, x1, this.a.ey(), rgba1);
        }
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            GuiDraw.drawRect(area.x, area.y, area.ex(), area.ey(), this.current == stop ? 0xffffffff : 0xff000000);
            GuiDraw.drawRect(area.x + 1, area.y + 1, area.ex() - 1, area.ey() - 1, this.fillColor(stop.color).getRGBAColor());
        }
