...
/**
 * Editor panel for {@link TrackerModifier} (S22 P240) — the modifier panel
 * that {@code GuiModifiersManager.PANELS} maps the tracker class to.
 *
 * <p>Verbatim port of the legacy panel: the tracker-name field (a plain
 * {@code GuiTextElement}, <b>not</b> the selector help field the Aperture
 * entity modifiers use — the value is a tracker morph's name, not an entity
 * selector), the x/y/z offset row, the yaw/pitch/roll row and the three
 * toggles. Every edit goes through {@code postUndo(undo(value, …))} so the
 * camera editor's undo stack sees it.</p>
 *
 * Legacy source: blockbuster-1.12/src/main/java/mchorse/blockbuster/aperture/gui/panels/modifiers/GuiTrackerModifierPanel.java
 */
public class GuiTrackerModifierPanel extends GuiAbstractModifierPanel<TrackerModifier>
{
    ...
    public GuiTrackerModifierPanel(MinecraftClient mc, TrackerModifier modifier, GuiModifiersManager modifiers)
    {
        ...
        this.fields.add(this.target,
            Elements.row(mc, 5, 0, 20, this.x, this.y, this.z),
            Elements.row(mc, 5, 0, 20, this.yaw, this.pitch, this.roll),
            this.relative, this.mainCam, this.lookAt);
    }
