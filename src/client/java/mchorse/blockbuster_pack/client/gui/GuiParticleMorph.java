...
/**
 * Editor GUI for {@link ParticleMorph} (roadmap P163) — a two-panel editor over
 * the {@link GuiAbstractMorph} base:
 *
 * <ul>
 *   <li><b>General</b>: vanilla/morph mode circulate, frequency/duration/delay/
 *       cap trackpads, the 1.12.2 particle-type list, position + spread + speed
 *       + count, local-rotation toggle and the comma-separated int-args
 *       field.</li>
 *   <li><b>Morph</b>: nested morph edit, movement-type list, yaw/pitch/
 *       sequencer/random toggles, and fade/lifeSpan/maximum trackpads.</li>
 * </ul>
 *
 * <p>The 3D morph renderer is hidden — the particle preview is only the emitted
 * particles (legacy {@code renderer.setVisible(false)}).</p>
 *
 * <p>Legacy source:
 * {@code blockbuster-1.12/.../blockbuster_pack/client/gui/GuiParticleMorph.java}.</p>
 */
public class GuiParticleMorph extends GuiAbstractMorph<ParticleMorph>
{
    ...
    public GuiParticleMorph(MinecraftClient mc)
    {
        super(mc);
    ...
    public static class GuiParticleMorphGeneralPanel extends GuiMorphPanel<ParticleMorph, GuiParticleMorph>
    {
        ...
        public GuiParticleMorphGeneralPanel(MinecraftClient mc, GuiParticleMorph editor)
        {
            super(mc, editor);
            ...
            this.type = new GuiStringListElement(mc, (value) -> this.morph.vanillaType = value.get(0));
            this.type.background();

            for (String name : LegacyParticleTypes.legacyNames())
            {
                this.type.add(name);
            }

            for (String name : LegacyParticleTypes.PLACEHOLDERS)
            {
                this.type.add(name);
            }
        ...
        public void fillData(ParticleMorph morph)
        {
            ...
            this.type.setCurrentScroll(morph.vanillaType == null ? "" : morph.vanillaType);
    ...
    public static class GuiParticleMorphMorphPanel extends GuiMorphPanel<ParticleMorph, GuiParticleMorph>
    {
        ...
        public GuiParticleMorphMorphPanel(MinecraftClient mc, GuiParticleMorph editor)
        {
            super(mc, editor);

            /* S22 P225/P226: the nested morph edit, legacy body verbatim. The
             * host is reached through IMorphEditorHost (P58), which carries
             * nestEdit itself since V-N — this used to resolve the concrete
             * creative picker with an instanceof-guarded cast, which silently
             * no-op'd for any other host rather than crashing. Only a genuinely
             * absent host (headless) is a no-op now. */
            this.pickMorph = new GuiNestedEdit(mc, (editing) ->
            {
                ...
                morphs.nestEdit(particle.morph, editing, (morph) ->
                {
        ...
        /**
         * The host of this editor, or {@code null} when there is none (headless,
         * or before the host attaches). Any {@link IMorphEditorHost} will do —
         * the nested-edit surface is on the interface. Same shape as
         * {@code GuiSequencerMorph}'s helper.
         */
