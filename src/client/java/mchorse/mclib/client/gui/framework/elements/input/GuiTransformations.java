 ...
 *
 * <p>Port of McLib 2.4.3's {@code GuiTransformations} (roadmap P41). All
 * layout constants, the STATIC (shared across every editor) orientation
 * value, the delta-rotate trackpads that reset to 0 on release, and the
 * 9-double {@code [tx,ty,tz,sx,sy,sz,rx,ry,rz]} clipboard NBT contract are
 * verbatim. Clipboard parsing goes through
 * {@code NBTUtils.parseSnbtCompound} (tolerant of legacy 1.12.2 indexed
 * list SNBT); pasted lists with fewer than 9 doubles are silently rejected,
 * longer ones read their first 9 — like legacy.</p>
 */
public class GuiTransformations extends GuiElement
{
    ...
    public GuiTransformations(MinecraftClient mc)
    {
        super(mc);
    ...
    public GuiContextMenu createContextMenu(GuiContext context)
    {
        ...
        NbtList transforms = parseTransforms(GuiUtils.getClipboardString());
    ...
    /**
     * Parse a clipboard string into a ≥9-double transforms list, or null
     * (legacy {@code JsonToNBT.getTagFromJson("{Transforms:" + clipboard +
     * "}")} + {@code getTagList("Transforms", TAG_DOUBLE)} with the ≥9
     * check). Package-private static for headless tests.
     */
    static NbtList parseTransforms(String clipboard)
    {
        try
        {
    ...
    private void copyTransformations()
    {
        ...
        list.add(NbtDouble.of(this.tx.value));
        list.add(NbtDouble.of(this.ty.value));
        list.add(NbtDouble.of(this.tz.value));
        list.add(NbtDouble.of(this.sx.value));
        list.add(NbtDouble.of(this.sy.value));
        list.add(NbtDouble.of(this.sz.value));
        list.add(NbtDouble.of(this.rx.value));
        list.add(NbtDouble.of(this.ry.value));
        list.add(NbtDouble.of(this.rz.value));

        GuiUtils.setClipboardString(list.toString());
    }

    public void pasteAll(NbtList list)
    {
    ...
    public void pasteTranslation(NbtList list)
    {
    ...
    public void pasteScale(NbtList list)
    {
    ...
    public void pasteRotation(NbtList list)
    {
    ...
    private Vector3d getVector(NbtList list, int offset)
    {
    ...
    {
        GuiDraw.drawStringWithShadow(this.font, I18n.translate("mclib.gui.transforms.translate"), this.tx.area.x, this.tx.area.y - 12, 0xffffff);
        GuiDraw.drawStringWithShadow(this.font, I18n.translate("mclib.gui.transforms.scale"), this.sx.area.x, this.sx.area.y - 12, 0xffffff);
        GuiDraw.drawStringWithShadow(this.font, I18n.translate("mclib.gui.transforms.rotate"), this.rx.area.x, this.rx.area.y - 12, 0xffffff);
    ...
    public static class GuiRelativeTrackpadElement extends GuiTrackpadElement
    {
        ...
        public GuiRelativeTrackpadElement(MinecraftClient mc, Consumer<Double> callback, IKey label)
        {
            super(mc, callback);
            this.label = label;
    ...
    public static class GuiStaticTransformOrientation extends GuiCirculateElement
    {
        ...
        public GuiStaticTransformOrientation(MinecraftClient mc, Consumer<GuiCirculateElement> callback)
        {
            super(mc, callback);
        }
