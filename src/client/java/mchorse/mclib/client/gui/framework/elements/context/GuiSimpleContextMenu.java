...
/**
 * Port of McLib 2.4.3's {@code GuiSimpleContextMenu} (roadmap P37).
 * Width = max action width (min 100); height clamped to screen − 10 via a
 * Supplier so window resizes re-clamp; bounds-constrained to the root with
 * 5px padding.
 */
public class GuiSimpleContextMenu extends GuiContextMenu
{
    ...
    public GuiSimpleContextMenu(MinecraftClient mc)
    {
        super(mc);
    ...
    {
        public GuiActionListElement(MinecraftClient mc, Consumer<List<Action>> callback)
        {
            super(mc, callback);
    ...
    public static class Action
    {
        ...
        {
            return 28 + GuiDraw.textWidth(font, this.label.get());
        }

        public void draw(TextRenderer font, int x, int y, int w, int h, boolean hover, boolean selected)
        {
            this.drawBackground(font, x, y, w, h, hover, selected);

            GuiDraw.resetColor();
            this.icon.render(x + 2, y + h / 2, 0, 0.5F);
            GuiDraw.drawString(font, this.label.get(), x + 22, y + (h - GuiDraw.fontHeight(font)) / 2 + 1, 0xffffff);
        }

        protected void drawBackground(TextRenderer font, int x, int y, int w, int h, boolean hover, boolean selected)
        {
            if (hover)
            {
                GuiDraw.drawRect(x, y, x + w, y + h, ColorUtils.HALF_BLACK + McLib.primaryColor.get());
            }
        }
    }
    ...
    public static class ColorfulAction extends Action
    {
        ...
        protected void drawBackground(TextRenderer font, int x, int y, int w, int h, boolean hover, boolean selected)
        {
            ...
            GuiDraw.drawRect(x, y, x + 2, y + h, 0xff000000 + this.color);
