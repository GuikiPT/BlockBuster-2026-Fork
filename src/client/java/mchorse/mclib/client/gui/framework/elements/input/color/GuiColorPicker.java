 ...
 *
 * (Port of McLib 2.4.3's {@code GuiColorPicker}, roadmap P34. Quirks kept:
 * the 7px slider inset is hardcoded for all sliders regardless of width;
 * {@code recentColors} is a static session-global list recorded on
 * close-by-click-outside; hex input accepts exactly 7 or 9 chars.)
 */
public class GuiColorPicker extends GuiElement
{
    ...
    /**
     * Legacy smooth-shaded two-triangle quad: opaque color in the top-left
     * triangle, the actual alpha toward the bottom-right (drawn over a
     * checkerboard). GL1 immediate mode → POSITION_COLOR triangles (P31
     * conventions); no-op headless.
     */
    public static void drawAlphaPreviewQuad(int x1, int y1, int x2, int y2, Color color)
    {
        ...
        vertexbuffer.begin(VertexFormat.DrawMode.TRIANGLES, VertexFormats.POSITION_COLOR);
        vertexbuffer.vertex(m, x1, y1, 0).color(color.r, color.g, color.b, 1).next();
        vertexbuffer.vertex(m, x1, y2, 0).color(color.r, color.g, color.b, 1).next();
        vertexbuffer.vertex(m, x2, y1, 0).color(color.r, color.g, color.b, 1).next();
        vertexbuffer.vertex(m, x2, y1, 0).color(color.r, color.g, color.b, color.a).next();
        vertexbuffer.vertex(m, x1, y2, 0).color(color.r, color.g, color.b, color.a).next();
        vertexbuffer.vertex(m, x2, y2, 0).color(color.r, color.g, color.b, color.a).next();
        tessellator.draw();

        RenderSystem.disableBlend();
    }

    public GuiColorPicker(MinecraftClient mc, Consumer<Integer> callback)
    {
        super(mc);
        ...
        this.recent.context(() ->
        {
            ...
            return new GuiSimpleContextMenu(MinecraftClient.getInstance())
    ...
    public void draw(GuiContext context)
    {
        if (this.dragging >= 0)
        {
            /* The 7px inset is a hardcoded contract for all four sliders */
        ...
        {
            GuiDraw.drawString(this.font, FAVORITE.get(), this.favorite.area.x, this.favorite.area.y - 10, 0x888888);
        }

        if (!this.recent.colors.isEmpty())
        {
            GuiDraw.drawString(this.font, RECENT.get(), this.recent.area.x, this.recent.area.y - 10, 0x888888);
        }
