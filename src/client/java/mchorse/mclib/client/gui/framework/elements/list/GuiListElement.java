 ...
 *
 * (Port of McLib 2.4.3's {@code GuiListElement}, roadmap P35. Contracts kept:
 * filtering preserves original indices; multi-select is Shift-click toggle;
 * drag-reorder arms after a 100ms hold; drop index -2 maps to the end and -1
 * aborts; selection keeps element identity across {@code sort()}.)
 */
public abstract class GuiListElement<T> extends GuiElement
{
    ...
    public GuiListElement(MinecraftClient mc, Consumer<List<T>> callback)
    {
        super(mc);
    ...
    public boolean mouseClicked(GuiContext context)
    {
        ...
        if (this.scroll.isInside(context) && context.mouseButton < 2)
        {
            ...
            if (this.exists(index))
            {
                if (this.multi && GuiUtils.isShiftKeyDown())
                {
    ...
    public void drawListElement(T element, int i, int x, int y, boolean hover, boolean selected)
    {
        if (selected)
        {
            if (this.isHorizontal())
            {
                GuiDraw.drawRect(x, y, x + this.scroll.scrollItemSize, y + this.scroll.h, ColorUtils.HALF_BLACK + McLib.primaryColor.get());
            }
            else
            {
                GuiDraw.drawRect(x, y, x + this.scroll.w, y + this.scroll.scrollItemSize, ColorUtils.HALF_BLACK + McLib.primaryColor.get());
            }
        }
    ...
    {
        GuiDraw.drawStringWithShadow(this.font, this.elementToString(element), x + 4, y + this.scroll.scrollItemSize / 2 - GuiDraw.fontHeight(this.font) / 2, hover ? 16777120 : 0xffffff);
    }
