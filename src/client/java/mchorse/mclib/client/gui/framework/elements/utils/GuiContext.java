...
/**
 * Port of McLib 2.4.3's {@code GuiContext} (roadmap P29). All int-based
 * mouse/keyboard state is preserved verbatim; the vanilla bridge
 * ({@link GuiBase}) converts 1.20.4's double coordinates and GLFW keycodes
 * at the boundary.
 *
 * Additive 1.20.4-only fields:
 * - {@link #drawContext} — the frame's {@link DrawContext}, stored by
 *   {@code GuiBase.render} so P31's {@code GuiDraw} can reach vanilla
 *   drawing.
 * - {@link #repeatEvents} — replacement for LWJGL2's global
 *   {@code Keyboard.enableRepeatEvents}; GLFW always sends repeats, so
 *   {@code GuiBase} drops them unless a text element (P33) sets this flag.
 */
public class GuiContext implements IViewportStack
{
    ...
    /**
     * The current frame's vanilla draw context (1.20.4 bridge, additive) —
     * set by {@code GuiBase.render} before the root is drawn.
     */
    public DrawContext drawContext;

    /**
     * Whether key-repeat events should be delivered (1.20.4 bridge,
     * additive). Text elements set this on focus (P33), mirroring legacy
     * {@code Keyboard.enableRepeatEvents(true)}.
     */
    ...
    public GuiContext(GuiBase screen)
    {
        ...
        this.keybinds = new GuiKeybinds(MinecraftClient.getInstance());
    ...
    /**
     * Legacy quirk (kept verbatim): unlike {@link GuiViewportStack}, the
     * context's {@code getShiftX/getShiftY} return the mouse coordinates.
     */
    @Override
