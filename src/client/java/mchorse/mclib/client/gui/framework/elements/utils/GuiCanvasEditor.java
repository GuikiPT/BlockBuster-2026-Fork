...
/**
 * Port of McLib 2.4.3's {@code GuiCanvasEditor} (roadmap P45) — a
 * {@link GuiCanvas} with a right-bottom 130px control column and a
 * checkerboard-backed frame area.
 *
 * <p>1.20.4 mapping: {@code Gui.drawRect} →
 * {@link GuiDraw#drawRect}; {@code GlStateManager.color(1,1,1,1)} →
 * {@link GuiDraw#resetColor()}; the legacy {@code alphaFunc(GL_GREATER,
 * 0/0.1F)} pair around the frame draw is dropped (no alpha test on the core
 * profile — blending covers it). {@link #setSize(int, int)} keeps the legacy
 * auto-fit that equalizes X/Y zoom to the smaller of the two.</p>
 */
public abstract class GuiCanvasEditor extends GuiCanvas
{
    ...
    public GuiCanvasEditor(MinecraftClient mc)
    {
        super(mc);
    ...
    protected void drawCanvas(GuiContext context)
    {
        ...
        /* Legacy computed a 16px-offset "processed" area here but rendered
         * the checkerboard over the whole frame area anyway — reproduced
         * verbatim (harmless, kept for diff-ability) */
