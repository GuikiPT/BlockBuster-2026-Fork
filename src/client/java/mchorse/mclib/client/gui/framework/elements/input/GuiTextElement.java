...
import mchorse.mclib.client.gui.utils.GuiResponder;
 ...
 *
 * (Port of McLib 2.4.3's {@code GuiTextElement}, roadmap P33.
 * {@code setText} resets the cursor to zero — pickers that programmatically
 * fill fields rely on the view showing the string start.)
 */
public class GuiTextElement extends GuiBaseTextElement implements GuiResponder, ITextColoring
{
    ...
    public GuiTextElement(MinecraftClient mc, ValueString value)
    {
        this(mc, value, null);
    }

    public GuiTextElement(MinecraftClient mc, ValueString value, Consumer<String> callback)
    {
    ...
    public GuiTextElement(MinecraftClient mc, int maxLength, Consumer<String> callback)
    {
        this(mc, callback);
        this.field.setMaxStringLength(maxLength);
    }

    public GuiTextElement(MinecraftClient mc, Consumer<String> callback)
    {
        super(mc);
    ...
    public void resize()
    {
        ...
        {
            this.field.x += 4;
            this.field.y += ((this.area.h - GuiDraw.fontHeight(this.font)) / 2);
            this.field.width -= 8;
            this.field.height = GuiDraw.fontHeight(this.font);
        }
    }
    ...
    public boolean keyTyped(GuiContext context)
    {
        if (this.isFocused())
        {
            if (context.keyCode == LegacyKeyCodes.KEY_TAB)
            {
