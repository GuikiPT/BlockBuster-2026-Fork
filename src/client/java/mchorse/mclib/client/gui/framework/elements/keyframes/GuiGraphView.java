 ...
 *
 * <p>Port of McLib 2.4.3's {@code GuiGraphView} (roadmap P43). Pinned legacy
 * behaviors: bezier-handle picking within a 16 px² squared distance, handle
 * mirroring to the opposite side unless Alt is held, Shift/Ctrl axis-locks
 * while dragging and axis-selective wheel zoom, {@code resetView()}
 * centering when the value range is flat (&lt; 0.01) with the −10/+10
 * fallback, curve tessellation at 10 segments (30 for BOUNCE/ELASTIC), and
 * the last keyframe never showing its right handle.</p>
 */
public class GuiGraphView extends GuiKeyframeElement
{
    ...
    public GuiGraphView(MinecraftClient mc, Consumer<Keyframe> callback)
    {
        super(mc, callback);
    ...
    {
        boolean x = GuiUtils.isShiftKeyDown();
        boolean y = GuiUtils.isCtrlKeyDown();
        boolean none = !x && !y;
    ...
    protected void drawGraph(GuiContext context, int mouseX, int mouseY)
    {
        ...
        {
            /* Headless — geometry only exists on a real frame */
            return;
        }
        ...
        for (Keyframe frame : channel.getKeyframes())
        {
            if (prev != null)
            {
                ...
                {
                    vb.vertex(m, px, this.toGraphY(prev.value), 0).color(r, g, b, 1F).next();
                    vb.vertex(m, fx, this.toGraphY(frame.value), 0).color(r, g, b, 1F).next();
                }
                else
                {
                    float seg = 10;
                    ...
                    for (int i = 0; i < seg; i++)
                    {
                        vb.vertex(m, px + (fx - px) * (i / seg), this.toGraphY(prev.interpolate(frame, i / seg)), 0).color(r, g, b, 1F).next();
                        vb.vertex(m, px + (fx - px) * ((i + 1) / seg), this.toGraphY(prev.interpolate(frame, (i + 1) / seg)), 0).color(r, g, b, 1F).next();
                    }
                }

                if (prev.interp == KeyframeInterpolation.BEZIER)
                {
                    vb.vertex(m, this.toGraphX(frame.tick - frame.lx), this.toGraphY(frame.value + frame.ly), 0).color(r, g, b, 0.6F).next();
                    vb.vertex(m, this.toGraphX(frame.tick), this.toGraphY(frame.value), 0).color(r, g, b, 0.6F).next();
                }
            }

            if (prev == null)
            {
                vb.vertex(m, 0, this.toGraphY(frame.value), 0).color(r, g, b, 1F).next();
                vb.vertex(m, this.toGraphX(frame.tick), this.toGraphY(frame.value), 0).color(r, g, b, 1F).next();
            }

            if (frame.interp == KeyframeInterpolation.BEZIER && index != count - 1)
            {
                vb.vertex(m, this.toGraphX(frame.tick), this.toGraphY(frame.value), 0).color(r, g, b, 0.6F).next();
                vb.vertex(m, this.toGraphX(frame.tick + frame.rx), this.toGraphY(frame.value + frame.ry), 0).color(r, g, b, 0.6F).next();
            }
        ...
        vb.vertex(m, this.toGraphX(prev.tick), this.toGraphY(prev.value), 0).color(r, g, b, 1F).next();
        vb.vertex(m, this.area.ex(), this.toGraphY(prev.value), 0).color(r, g, b, 1F).next();
    ...
    protected Keyframe moving(GuiContext context, int mouseX, int mouseY)
    {
        ...
        else
        {
            ...
            if (GuiUtils.isShiftKeyDown()) x = this.lastT;
            if (GuiUtils.isCtrlKeyDown()) y = this.lastV;
            ...
            this.setTick(x, !GuiUtils.isAltKeyDown());
            this.setValue(y, !GuiUtils.isAltKeyDown());
        }
