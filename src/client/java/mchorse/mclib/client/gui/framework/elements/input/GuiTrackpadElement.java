...
/**
 * Port of McLib 2.4.3's {@code GuiTrackpadElement} (roadmap P33) — the
 * drag-number input every editor is built on. All value math is verbatim.
 *
 * 1.20.4 boundary decisions:
 * - Drag cursor wrapping across screen edges: {@code Mouse.setCursorPosition}
 *   → {@code GLFW.glfwSetCursorPos} on the window handle; the legacy 30ms
 *   {@code Timer} guard and the {@code shiftX} virtual-distance accumulator
 *   are kept as-is.
 * - Modifier polling ({@code Keyboard.isKeyDown(KEY_LSHIFT/…)}) →
 *   {@code GuiUtils.isKeyDown} with the same left-key-only scancodes.
 */
public class GuiTrackpadElement extends GuiBaseTextElement
{
    ...
    /**
     * Decimal separator symbols for every formatter in this class.
     *
     * <p>Deliberately {@link Locale#ROOT} rather than the JVM default: the
     * formatted string is fed straight back into {@link Double#valueOf} (see
     * {@link #setValueInternal(double)}) and typed into the text field that
     * {@code Double.parseDouble} reads. On a comma-decimal locale (de_DE,
     * fr_FR, pt_BR, …) the default symbols produce {@code "0,9375"}, which
     * {@code Double} cannot parse — every fractional value crashed the model
     * editor on open. {@code Locale.ROOT} is identical to the en_US output the
     * 1.12.2 build produced, so no displayed number changes.</p>
     */
    ...
    public GuiTrackpadElement(MinecraftClient mc, ValueInt value)
    {
        this(mc, value, null);
    }

    public GuiTrackpadElement(MinecraftClient mc, ValueInt value, @Nullable Consumer<Double> callback)
    {
    ...
    public GuiTrackpadElement(MinecraftClient mc, ValueLong value)
    {
        this(mc, value, null);
    }

    public GuiTrackpadElement(MinecraftClient mc, ValueLong value, @Nullable Consumer<Double> callback)
    {
    ...
    public GuiTrackpadElement(MinecraftClient mc, ValueFloat value)
    {
        this(mc, value, null);
    }

    public GuiTrackpadElement(MinecraftClient mc, ValueFloat value, @Nullable Consumer<Double> callback)
    {
    ...
    public GuiTrackpadElement(MinecraftClient mc, ValueDouble value)
    {
        this(mc, value, null);
    }

    public GuiTrackpadElement(MinecraftClient mc, ValueDouble value, @Nullable Consumer<Double> callback)
    {
    ...
    public GuiTrackpadElement(MinecraftClient mc, @Nullable Consumer<Double> callback)
    {
        super(mc);
    ...
    public boolean mouseClicked(GuiContext context)
    {
        ...
        if (context.mouseButton == 0)
        {
            ...
            {
                if (GuiUtils.isKeyDown(Keys.KEY_LCONTROL))
                {
    ...
    public boolean keyTyped(GuiContext context)
    {
        ...
        if (this.isFocused())
        {
            if (context.keyCode == LegacyKeyCodes.KEY_UP)
            {
                this.setValueAndNotify(this.value + this.getValueModifier());

                return true;
            }
            else if (context.keyCode == LegacyKeyCodes.KEY_DOWN)
            {
                this.setValueAndNotify(this.value - this.getValueModifier());

                return true;
            }
            else if (context.keyCode == LegacyKeyCodes.KEY_TAB)
            {
            ...
            else if (context.keyCode == LegacyKeyCodes.KEY_RETURN && GuiUtils.isAltKeyDown())
            {
                try
                {
    ...
    public void draw(GuiContext context)
    {
        ...
        {
            /* Legacy dropped the alpha test to 0 for the faint arrow-strip
             * quads; core profile has no alpha test — plain blending covers it */
            this.plusOne.draw(plus ? 0x22ffffff : 0x0affffff, padding);
            this.minusOne.draw(minus ? 0x22ffffff : 0x0affffff, padding);

            GuiDraw.bindColor(minus ? 0xffffffff : 0x80ffffff);
            Icons.MOVE_LEFT.render(x + 5, y + (h - 16) / 2);
            GuiDraw.bindColor(plus ? 0xffffffff : 0x80ffffff);
            Icons.MOVE_RIGHT.render(x + w - 13, y + (h - 16) / 2);
            GuiDraw.resetColor();
        }

        int width = MathUtils.clamp(GuiDraw.textWidth(this.font, this.field.getText()), 0, w - 16);
        ...
        if (dragging)
        {
            ...
            /* Mouse doesn't change immediately the next frame after
             * glfwSetCursorPos(), so this is a hack that stops double
             * shifting (legacy comment; the 30ms Timer guard) */
            if (this.changed.isTime())
            {
                ...
                {
                    double factor = Math.ceil(this.mc.getWindow().getWidth() / (double) context.screen.width);
                    long handle = this.mc.getWindow().getHandle();

                    if (mouseX <= border)
                    {
                        GLFW.glfwSetCursorPos(handle, this.mc.getWindow().getWidth() - (int) (factor * borderPadding), this.mc.mouse.getY());
                    ...
                    {
                        GLFW.glfwSetCursorPos(handle, (int) (factor * borderPadding), this.mc.mouse.getY());
    ...
    protected double getValueModifier()
    {
        ...
        if (GuiUtils.isKeyDown(Keys.KEY_LSHIFT))
        {
            value = this.strong;
        }
        else if (GuiUtils.isKeyDown(Keys.KEY_LCONTROL))
        {
            value = this.increment;
        }
        else if (GuiUtils.isKeyDown(Keys.KEY_LMENU))
        {
            value = this.weak;
        }
    ...
    private DecimalFormat getRoundingFormat(boolean force)
    {
        if (this.decimalPlaces != McLib.trackpadDecimalPlaces.get() || force) //to save performance
        {
            ...
            this.rounding = new DecimalFormat("#." + decimals, SYMBOLS);
            this.decimalPlaces = McLib.trackpadDecimalPlaces.get();
