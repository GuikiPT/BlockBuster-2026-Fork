 ...
 * channels)
 *
 * <p>Port of McLib 2.4.3's {@code GuiDopeSheet} (roadmap P43). Pinned legacy
 * behaviors: {@code TOP_MARGIN = 15} row layout, the 4 px euclidean pick
 * radius, Alt+click picking across all rows at any Y, double-click inserting
 * at the channel-interpolated value while inheriting the previous keyframe's
 * easing/interpolation, and the long-cast of multi-selection tick deltas
 * (per-keyframe rounding drift is legacy behavior).</p>
 */
public class GuiDopeSheet extends GuiKeyframeElement
{
    ...
    public GuiDopeSheet(MinecraftClient mc, Consumer<Keyframe> callback)
    {
        super(mc, callback);
    }
    ...
    {
        /* P279: every row height below divides by the sheet count; an empty
         * dope sheet (no channels on the selected element) is reachable and
         * legacy divided by zero here. drawGraph already returned early. */
    ...
    {
        if (this.isGrabbing() && !this.sheets.isEmpty())
        {
            /* Multi select */
            Area area = new Area();
    ...
    protected void drawGraph(GuiContext context, int mouseX, int mouseY)
    {
        ...
        boolean gl = GuiDraw.getDrawContext() != null;
        Matrix4f m = matrix();
        ...
        for (GuiSheet sheet : this.sheets)
        {
            ...
            if (gl)
            {
                ...
                vb.vertex(m, this.area.x, y + h / 2, 0).color(COLOR.r, COLOR.g, COLOR.b, 0.65F).next();
                vb.vertex(m, this.area.ex(), y + h / 2, 0).color(COLOR.r, COLOR.g, COLOR.b, 0.65F).next();
            ...
            /* Right-aligned gradient title tag (P31 GuiDraw — headless-safe) */
            int lw = GuiDraw.textWidth(this.font, sheet.title.get()) + 10;

            GuiDraw.drawHorizontalGradientRect(this.area.ex() - lw - 10, y, this.area.ex(), y + h, sheet.color, 0xaa000000 + sheet.color, 0);
            GuiDraw.drawStringWithShadow(this.font, sheet.title.get(), this.area.ex() - lw + 5, y + (h - GuiDraw.fontHeight(this.font)) / 2 + 1, 0xffffff);
