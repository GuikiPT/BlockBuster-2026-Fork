...
/**
 * Port of McLib 2.4.3's {@code GuiConfigPanel} (roadmap P44) — the config
 * dashboard panel: mod list on the left, per-category widget rows on the
 * right (horizontal-scroll columns, 240 px wide, exactly like 1.12.2).
 *
 * <p>Widget rows come from {@link ConfigGuiProviders} (the S3 factory
 * registry that absorbed the legacy per-value {@code getFields} bodies —
 * see that class for the type→widget table); client-side self-providing
 * values keep the legacy {@code instanceof IConfigGuiProvider} path.</p>
 *
 * <p>Server-config mode (the download button) requests configs over S2's
 * {@code PacketRequestConfigs} machinery; on close, pending server configs
 * are sent back (legacy behavior — {@code close()} re-triggers
 * {@code request()} which uploads when {@code serverConfigs != null}).</p>
 */
public class GuiConfigPanel extends GuiDashboardPanel<GuiAbstractDashboard>
{
    ...
    public GuiConfigPanel(MinecraftClient mc, GuiAbstractDashboard dashboard)
    {
        super(mc, dashboard);
    ...
    public void open()
    {
        this.request.setVisible(this.mc != null && !this.mc.isIntegratedServerRunning() && OpHelper.isPlayerOp());
    }
    ...
    public void refresh()
    {
        ...
        boolean isSingleplayer = this.mc != null && this.mc.isIntegratedServerRunning();
        ...
        for (Value category : this.config.values.values())
        {
            ...
            label.tooltip(IKey.lang(catTooltipKey), Direction.BOTTOM).flex().w(GuiDraw.textWidth(this.font, label.label.get()));
            ...
            for (Value value : category.getSubValues())
            {
                ...
                /* Legacy: instanceof IConfigGuiProvider; the port also
                 * consults the client-side factory registry (see
                 * ConfigGuiProviders) — unproviderable values are skipped
                 * either way */
                List<GuiElement> fields = ConfigGuiProviders.getFields(this.mc, this, value);
    ...
    public void draw(GuiContext context)
    {
        ...
        GuiDraw.drawStringWithShadow(this.font, this.title.get(), this.area.x + 10, this.area.y + 20 - GuiDraw.fontHeight(this.font) / 2, 0xffffff);
